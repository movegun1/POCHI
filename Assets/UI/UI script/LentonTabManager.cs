...
public class LentonTabManager : MonoBehaviour
{
    public Button[] lentonButtons; // 5개의 버튼
    public TextMeshProUGUI[] buttonTexts; // 각 버튼의 텍스트
    public int[] unlockCosts = { 0, 9900, 39000, 99000, 190000 }; // 해금에 필요한 금액
    ...
    {
        // 상태 배열 초기화
        buttonStates = new ButtonState[lentonButtons.Length];

        // 데이터 불러오기
        LoadState();

        // 버튼 초기화
        for (int i = 0; i < lentonButtons.Length; i++)
        {
            ...
            // 버튼 클릭 이벤트 추가
            lentonButtons[i].onClick.AddListener(() => OnButtonClick(index));
        }
    }
    ...
    private void UpdateButtonState(int index)
    {
        ...
        switch (buttonStates[index])
        {
            case ButtonState.Locked:
                ...
                lentonButtons[index].interactable = true; // 버튼 활성화
                break;
            ...
            case ButtonState.Equipped:
                ...
                lentonButtons[index].interactable = false; // 비활성화
                break;
        }
    }
    ...
    private void SaveState()
    {
        ...
        {
            // 각 버튼의 상태 저장
            PlayerPrefs.SetInt($"LentonButton{i}State", (int)buttonStates[i]);
        }

        // 현재 장착된 버튼 저장
        PlayerPrefs.SetInt("LentonEquippedIndex", currentEquippedIndex);
    ...
    private void LoadState()
    {
        for (int i = 0; i < buttonStates.Length; i++)
        {
            ...
            buttonStates[i] = (ButtonState)PlayerPrefs.GetInt($"LentonButton{i}State", (i == 0 ? (int)ButtonState.Unlocked : (int)ButtonState.Locked));
        }

        // 현재 장착된 버튼 불러오기 (기본값: -1)
        currentEquippedIndex = PlayerPrefs.GetInt("LentonEquippedIndex", -1);
    }
}
